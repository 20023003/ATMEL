 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void spd1_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTC_set_pin_pull_mode(4, pull_mode);
}

/**
 * \brief Set spd1 data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void spd1_set_dir(const enum port_dir dir)
{
	PORTC_set_pin_dir(4, dir);
}

/**
 * \brief Set spd1 level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void spd1_set_level(const bool level)
{
	PORTC_set_pin_level(4, level);
}

/**
 * \brief Toggle output level on spd1
 *
 * Toggle the pin level
 */
static inline void spd1_toggle_level()
{
	PORTC_toggle_pin_level(4);
}

/**
 * \brief Get level on spd1
 *
 * Reads the level on a pin
 */
static inline bool spd1_get_level()
{
