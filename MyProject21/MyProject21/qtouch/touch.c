...
/* Node configurations */
qtm_acq_m324pb_node_config_t ptc_seq_node_cfg1[DEF_NUM_CHANNELS]
...
static void touch_ptc_pin_config(void)
{
	...
	PORTA_set_pin_pull_mode(3, PORT_PULL_OFF);

	PORTA_set_pin_pull_mode(1, PORT_PULL_OFF);

	PORTA_set_pin_pull_mode(4, PORT_PULL_OFF);

	PORTA_set_pin_pull_mode(0, PORT_PULL_OFF);
...
/*============================================================================
void Timer_set_comp_a(uint8_t value)
------------------------------------------------------------------------------
...
------------------------------------------------------------------------------
Purpose: This function provides the 16 bit time interval from the configured
         8 bit time interval by hardware timer peripheral. Uses the internal
         static variable to count the instances of timer interrupts.
Input  : none
Output : none
Notes  :
============================================================================*/
void mega_touch_timer(void)
{
	volatile static uint16_t touch_time_counter = 0u;
	if (touch_time_counter == DEF_TOUCH_MEASUREMENT_PERIOD_MS) {
		touch_time_counter = 0;
		touch_timer_handler();
	} /*if*/
	else {
		touch_time_counter++;
